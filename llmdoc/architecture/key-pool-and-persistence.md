...
- Runtime MCP session migration persists the rebound primary affinity without revoking the session being migrated; `primary_api_key_rebound` revocation still applies to ordinary affinity changes, but the transparent migration path skips that side effect so later calls on the same proxy session remain valid. Source: `src/tavily_proxy/mod.rs:3709`, `src/tavily_proxy/mod.rs:3767`, `src/tavily_proxy/mod.rs:3791`, `src/server/tests.rs:20423`.
