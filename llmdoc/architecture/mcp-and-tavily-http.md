...
- Internal MCP session replays now send `Accept: application/json, text/event-stream` on the replayed `initialize` and `notifications/initialized` hops, matching the external MCP transport contract so upstream session rebuilds do not fail with `406 Not Acceptable`. Source: `src/tavily_proxy/mod.rs:4111`, `src/tavily_proxy/mod.rs:4304`, `src/tavily_proxy/mod.rs:4360`, `src/server/tests.rs:20075`.
- MCP retry classification now treats outer HTTP `429`, inner Tavily `status=429`, and `upstream_rate_limited_429` failure kinds as the same rate-limit signal, so `HTTP 200` responses that wrap a logical Tavily `429` still trigger cooldown and key migration. Source: `src/tavily_proxy/mod.rs:4092`, `src/tavily_proxy/mod.rs:4105`, `src/tavily_proxy/mod.rs:4514`.
- After a successful MCP replay, the proxy now continues the same client request with the in-memory migrated session state instead of blocking on an immediate database reread of the proxy session row. Source: `src/tavily_proxy/mod.rs:4402`, `src/tavily_proxy/mod.rs:4435`, `src/server/tests.rs:20075`.
- Runtime MCP session migration now uses a non-revoking affinity rebind path: it may persist the new primary key choice for the token or account, but it must not revoke the currently migrating proxy session with `primary_api_key_rebound`. Source: `src/tavily_proxy/mod.rs:3692`, `src/tavily_proxy/mod.rs:3709`, `src/tavily_proxy/mod.rs:4318`, `src/server/tests.rs:20294`.
