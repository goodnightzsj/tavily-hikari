...
- Automatic release only proceeds when the merged PR resolves to exactly one allowed intent label (`type:patch|minor|major`) and exactly one channel label (`channel:stable|rc`); `type:docs` and `type:skip` intentionally suppress image publication. Source: `.github/scripts/release-intent.sh:10`, `.github/scripts/release-intent.sh:38`, `.github/scripts/release-intent.sh:148`, `.github/scripts/release-intent.sh:194`.
