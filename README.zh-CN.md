...
- **生产级 CI/CD**：GitHub Actions 对代码格式、lint、单元测试把关；`main` 上每次成功提交后都会自动发版（打 tag + GitHub Release），`ghcr.io` 镜像改为手动发布。
...
- **Release**：`.github/workflows/release.yml` 在 main CI 通过后触发，默认只负责打 tag / 创建 Release；GHCR 镜像需手动触发发布。

## 发版

`main` 上每次成功提交后，Release workflow 都会自动发版：

- 固定按稳定版 patch 递增，计算下一版本号 `vX.Y.Z`。
- 自动创建 Git tag 与 GitHub Release。
- 如果需要推送 Docker 镜像，需要手动 dispatch `Release` workflow，并传入：
  - `head_sha=<目标 main commit>`
  - `publish_docker=true`
- 手动 Docker 发布时会推送 GHCR 镜像标签：`latest`、`vX.Y.Z`。
